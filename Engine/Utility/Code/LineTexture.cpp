...
Engine::CLineTexture::CLineTexture(LPDIRECT3DDEVICE9 ptr_device)
	: CVIBuffer(ptr_device)
{
}

Engine::CLineTexture::~CLineTexture()
{
}

Engine::CComponent * Engine::CLineTexture::CloneComponent()
{
	++reference_count_;
	return this;
}

Engine::CLineTexture * Engine::CLineTexture::Create(LPDIRECT3DDEVICE9 ptr_device)
{
	CLineTexture* ptr_buffer = new CLineTexture(ptr_device);
...
HRESULT Engine::CLineTexture::InitBuffer()
{
	...
	ptr_vertex[0].position = Vector3(0.f, 0.5f, 0.f);
	ptr_vertex[0].texture_uv = Vector2(0.f, 0.f);

	ptr_vertex[1].position = Vector3(0.f, 0.5f, 1.f);
	ptr_vertex[1].texture_uv = Vector2(1.f, 0.f);

	ptr_vertex[2].position = Vector3(0.f, -0.5f, 1.f);
	ptr_vertex[2].texture_uv = Vector2(1.f, 1.f);

	ptr_vertex[3].position = Vector3(0.f, -0.5f, 0.f);
